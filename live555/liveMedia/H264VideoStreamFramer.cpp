...
void LiveSourceWithx264::deliverFrame()
{
    ...
    if ( nal.p_payload[0] == 0 && nal.p_payload[1] == 0 && nal.p_payload[2] == 0 && nal.p_payload[3] == 1&& (nal[4].p_payload & 0x1F)==8 )
    {
    	bDetectPPS=false;
    	//printf("bDetectPPS=false;\n");
    }
    if( ( nal.p_payload[0] == 0 && nal.p_payload[1] == 0 && nal.p_payload[2] == 1 && (nal[3].p_payload & 0x1F)==5)||
    		 ( nal.p_payload[0] == 0 && nal.p_payload[1] == 0 && nal.p_payload[2] == 0 && nal.p_payload[3] ==1 && (nal[4].p_payload & 0x1F)==5))
    {
    	bDetectIDR=false;
