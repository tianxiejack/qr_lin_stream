...
int OSA_i2cTestShowUsage(char *str)
{
	  ...
	  printf(" Usage: %s -r|-w |-h,[<i2c dev>] <devAddrInHex> <regAddrInHex> <regValueInHex or numRegsToReadInDec> \r\n", str);
	  printf(" \n");
	  return 0;
}
...
int OSA_i2cTestMain(int argc, char **argv)
{
	...
	regValue8[0] = 0;
	regAddr[0] = 0;
	i2cDevId    = I2C_DEFAULT_INST_ID;
	
	/* added i2c device input 
	*   changed by xavier
	*/
	if(argc>5 ){
		if(strcmp(argv[2], "i2c1")==0)
			i2cDevId =I2C_1_INST_ID;
		else if(strcmp(argv[2], "i2c0")==0)
			i2cDevId =I2C_0_INST_ID;

		devAddr = xstrtoi(argv[3]);
		regAddr[0] = xstrtoi(argv[4]);
		if(doRead){
			  numRegs = atoi(argv[5]);
	...
	status = OSA_i2cOpen(&i2cHndl, i2cDevId);
