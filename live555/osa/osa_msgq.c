...
/******************************************************************************************************** 
  Function:		OSA_msgqRecvMsg(OSA_MsgqHndl *hndl, OSA_MsgHndl *msg, Uint32 timeout)
  Description:  rewrite the OSA_msgqRecvMsg, in this function, OSA_MsgHndl message mem is free after the 
  				message is read from message queue.
  Calls:        
  Called By:  	
  Input:        OSA_MsgqHndl *hndl, OSA_MsgHndl *msg, Uint32 timeout
  Output:      	null  
  Return:      	0
  Others:       
**********************************************************************************************************/ 
int OSA_msgqRecvMsgEx(OSA_MsgqHndl *hndl, OSA_MsgHndl *msg, Uint32 timeout)
{
  int status = OSA_EFAIL;
  ...
  while(1) {
    if(hndl->count > 0 ) {
      ...
      if(msg!=NULL) {
		...
		memcpy(msg,hndl->queue[hndl->curRd],sizeof(OSA_MsgHndl));
		OSA_msgqFreeMsgHndl(hndl->queue[hndl->curRd]);
      }
